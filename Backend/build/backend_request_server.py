...
def create_token():
    ...
    user = db_search.lookupuser(username, password)
    if not (user):
...
def usertable():
    ...
    user = db_search.lookupuser(username, password)
    if not (user):
        create = db_search.create(username, name, password)
        access_token = create_access_token(identity=username)
        response = {"access_token":access_token}
        return response
    elif user:
...
def search():
    ...
    findings = db_search.search(activity, date, guests, budget)
    db_search.validation(activity, date, budget, guests)
    return findings
...
def time():
    ...
    available_spots = db_search.gettimes(id, date)
...
def table():
    db_search.places()
    return app.logger.info('success data upload')

@app.route('/validation')
def validation():
    db_search.records()
    return app.logger.info('success validation')
